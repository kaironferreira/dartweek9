...
class HomeController extends Cubit<HomeState> {
  ...
  void addOrUpdateBag(OrderProductDto orderProduct) {
    final shoppingBar = [...state.shoppingBag];
    final orderIndex = shoppingBar
        .indexWhere((orderP) => orderP.product == orderProduct.product);

    if (orderIndex > -1) {
      if (orderProduct.amount == 0) {
        shoppingBar.removeAt(orderIndex);
      } else {
        shoppingBar[orderIndex] = orderProduct;
      }
    } else {
      shoppingBar.add(orderProduct);
    }

    emit(state.copyWith(shoppingBag: shoppingBar));
  }
